...
extension RTMPStream: RTMPMuxerDelegate {
    public func sampleOutput(muxer:RTMPMuxer, audio buffer:NSData, timestamp:Double) {
    ...
    public func sampleOutput(muxer:RTMPMuxer, video buffer:NSData, timestamp:Double) {
        guard readyState == .Publishing else {
            return
        }
